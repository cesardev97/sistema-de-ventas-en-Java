...
public class Crud_VentaPDF {
    ...
    private int leerNumeroFactura() {
        int numero = 1;
        try (BufferedReader reader = new BufferedReader(new FileReader("src/pdf/numeroFactura.txt"))) {
            numero = Integer.parseInt(reader.readLine());
        } catch (IOException e) {
            System.out.println("Error al leer el número de factura: " + e);
        }
        return numero;
    }

    private void guardarNumeroFactura(int numero) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("src/pdf/numeroFactura.txt"))) {
            writer.write(String.valueOf(numero));
        } catch (IOException e) {
            System.out.println("Error al guardar el número de factura: " + e);
        }
    }
    ...
    public void generarFacturaPDF() {
        try {
            ...
            fecha.add("Factura: " + String.format("%03d", numeroFactura) + "\nFecha: " + fechaActual + "\nHora: " + horaActual + "\n\n");
